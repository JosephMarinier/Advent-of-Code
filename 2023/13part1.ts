fetch(`${window.location.pathname}/input`).then((r) => r.text()).then((text) => {
    const answer = text.trim().split("\n\n").map((pattern) => {
        ...
        const [horizontalMirror, verticalMirror] = [rows, columns].map((array) => 1 + array.findIndex((_, index) => {
            const mirror = index + 1;
            if (mirror === array.length) {
                return false
            };
        ...
        return 100 * horizontalMirror + verticalMirror;
    }).reduce((a, b) => a + b);
