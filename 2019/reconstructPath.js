reconstructPath = (cameFrom, current) => {
    ...
    while (cameFrom.has(current)) {
        path.unshift(cameFrom.get(current)[current.indexOf(`@`)]);
    ...
    while (openSet.size > 0) {
        ...
        neighbors(current).forEach(([neighbor, d]) => {
            ...
            if (!gScore.has(neighbor) || tentative_gScore < gScore.get(neighbor)) {
                ...
                const g = goal(neighbor);
                if (i % 100 === 0 || g) {
                    console.log(i, openSet.size, tentative_gScore, heu, g);
                }
    ...
    while (answer < 1000 && progress > 0) {
        ...
        map.forEach((line, y) => {
            line.forEach((pixel, x) => {
                if (distances[y][x] === Infinity && /[\.a-z]/.test(pixel) && [
...
########################`;
    ...
    const answer = search(text, (text) => /^[^a-z]+$/.test(text), (text) => text.replace(/[^a-z]/g, ``).length);
