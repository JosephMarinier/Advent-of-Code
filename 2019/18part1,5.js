...
fetch(`${window.location.pathname}/input`).then((r) => r.text()).then((text) => {
    ...
    time(() => {
        answer = search(text, (text) => text.match(/[a-z]/) === null, (text) => {
            const keys = text.match(/[a-z]/g);
            if (keys === null) {
                return 0;
            }
            const current = pos(`@`);
            const poses = keys.map(pos);
            const ds = poses.map(({x, y}) => {
                return Math.abs(x - current.x) + Math.abs(y - current.y);
            });
            return ds.reduce((a, b) => Math.min(a, b)) + keys.length - 1;
            // return ds.reduce((a, b) => Math.max(a, b));
            // const min = poses.reduce((a, b) => ({x: Math.min(a.x, b.x), y: Math.min(a.y, b.y)}));
            // const max = poses.reduce((a, b) => ({x: Math.max(a.x, b.x), y: Math.max(a.y, b.y)}));
            // return ds.reduce((a, b) => Math.min(a, b)) + max.x + max.y - min.x - min.y;
        });
    });
